...
from repository import models, config
from . import agorithm
...
def get_untreated_servers():
    ...
    try:
        ...
        # 今日未采集的资产
        con_datetime = Q()
        con_datetime.connector = 'OR'
        con_datetime.children.append(("asset__update_time__lt", current_datetime))  # 资产更新时间小于当前时间
        con_datetime.children.append(("asset__update_time", None))  # 资产更新时间为null

        # 在线状态的服务器
        con_status = Q()
        con_status.children.append(('asset__status', '0'))

        condition = Q()
        condition.add(con_datetime, 'AND')
        condition.add(con_status, 'AND')

        result = models.Server.objects.filter(condition).values('hostname')  # <QuerySet>
        response.data = list(result)
        response.status = True
    except Exception as e:
        response.status = False
        response.error = str(e)
        models.ErrorLog.objects.create(asset_id=None, title='get_untreated_servers', content=traceback.format_exc())
    return response
...
class HandleBasic(object):
    # 处理基本信息，包括主板和CPU信息
    @staticmethod
    def process(server_obj, server_info, user_obj=None):
        response = BaseResponse()
        try:
            log_list = []
            main_board = server_info['main_board']['data']
            cpu = server_info['cpu']['data']
            if server_obj.os_type != server_info['os_platform']:
                log_list.append('系统由%s变更为%s' % (server_obj.os_type, server_info['os_platform'],))
                server_obj.os_type = server_info['os_platform']
            ...
            if server_obj.server_model != main_board['model']:
                log_list.append(u'主板型号由%s变更为%s' % (server_obj.server_model, main_board['model'],))
                server_obj.server_model = main_board['model']
            ...
            server_obj.save()
            if log_list:
                models.AssetRecord.objects.create(asset=server_obj.asset, creator=user_obj,
                                                  content=';'.join(log_list), record_type=config.HARDWARE_CHANGE)
        except Exception as e:
            response.status = False
    ...
    @staticmethod
    def update_last_time(server_obj, user_obj=None):
        response = BaseResponse()
        try:
            current_datetime = datetime.datetime.today()
            server_obj.asset.update_time = current_datetime
            server_obj.asset.save()
            models.AssetRecord.objects.create(asset=server_obj.asset, creator=user_obj,
                                              content='资产汇报', record_type=config.ASSET_REPORT)
        except Exception as e:
            response.status = False
            models.ErrorLog.objects.create(asset=server_obj.asset, title='basic-run',
                                           content=traceback.format_exc())
...
class HandleNic(object):
    ...
    def process(server_obj, server_info, user_obj=None):
        ...
        try:
            ...
            if not nic_info['status']:
                response.status = False
                models.ErrorLog.objects.create(asset=server_obj.asset, title='nic-agent', content=nic_info['error'])
                return response
            # 新的网卡数据
            client_nic_dict = nic_info['data']
            nic_obj_list = models.Nic.objects.filter(server=server_obj)
            # 数据库中的网卡数据
    ...
    def _add_nic(add_list, client_nic_dict, server_obj, user_obj):
        for item in add_list:
            cur_nic_dict = client_nic_dict[item]  # 单个网卡数据
            cur_nic_dict['name'] = item
            log_str = '[新增网卡]{name}:mac地址为{mac_addr};状态为{up};掩码为{netmask};IP地址为{ip_addr}'.format(**cur_nic_dict)
            cur_nic_dict['server'] = server_obj
            models.Nic.objects.create(**cur_nic_dict)
            models.AssetRecord.objects.create(asset=server_obj.asset, creator=user_obj,
                                              content=log_str, record_type=config.HARDWARE_CHANGE)
    ...
    def _del_nic(del_list, nic_objs, server_obj, user_obj):
        for item in nic_objs:
            if item.name in del_list:
                log_str = '[移除网卡]{name}:mac地址为{mac_addr};状态为{up};掩码为{netmask};IP地址为{ip_addr}'.format(**item.__dict__)
                item.delete()
                models.AssetRecord.objects.create(asset=server_obj.asset, creator=user_obj,
                                                  content=log_str, record_type=config.HARDWARE_CHANGE)
    ...
    def _update_nic(update_list, nic_objs, client_nic_dict, server_obj, user_obj):
        ...
        for item in nic_objs:
            if item.name in update_list:
                ...
                new_mac_addr = client_nic_dict[item.name]['mac_addr']
                if item.mac_addr != new_mac_addr:
                    log_list.append(u"[更新网卡]%s:mac地址由%s变更为%s" % (item.name, item.hwaddr, new_mac_addr))
                    item.mac_addr = new_mac_addr
                ...
                new_ip_addr = client_nic_dict[item.name]['ip_addr']
                if item.ip_addr != new_ip_addr:
                    log_list.append(u"[更新网卡]%s:IP地址由%s变更为%s" % (item.name, item.ipaddrs, new_ip_addr))
                    item.ip_addr = new_ip_addr
    ...
    def process(server_obj, server_info, user_obj=None):
        ...
        try:
            ...
            if not mem_info['status']:
                models.ErrorLog.objects.create(asset=server_obj.asset, title='memory-agent',
            ...
            mem_obj_list = models.RAM.objects.filter(server=server_obj)
            ...
            response.status = False
            models.ErrorLog.objects.create(asset=server_obj.asset, title='memory-run',
                                           content=traceback.format_exc())
    ...
    def _add_memory(add_list, client_mem_dict, server_obj, user_obj):
        for item in add_list:
            ...
            log_str = '[新增内存]插槽为{slot};容量为{capacity};类型为{model};速度为{speed};厂商为{manufacturer};SN号为{sn}'.format(
                **cur_mem_dict)
            cur_mem_dict['server'] = server_obj
...
class HandleDisk(object):
    ...
    def process(server_obj, server_info, user_obj=None):
        ...
        try:
            ...
            if not disk_info['status']:
                response.status = False
                models.ErrorLog.objects.create(asset=server_obj.asset, title='disk-agent',
                                               content=disk_info['error'])
                return response

            client_disk_dict = disk_info['data']

            disk_obj_list = models.Disk.objects.filter(server=server_obj)
    ...
    def _add_disk(add_list, client_disk_dict, server_obj, user_obj):
        for item in add_list:
            ...
            cur_disk_dict['server'] = server_obj
