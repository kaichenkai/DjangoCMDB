...
from lib.serializer import Json
...
class BaseClient(object):
    ...
    def get_asset(self):
        ...
        try:
            ...
            # response.json()方法等同于json.loads（response.text）方法
            # return response.json()
            return json.loads(response.text)
        except Exception as e:
            task = {"message": "请求 SSH 远程连接数据失败", "detail": str(e), "status": False}
            return task
...
class SSHClient(BaseClient):
    def process(self):
        ...
        task = self.get_asset()
        if not task['status']:
            Logger().log(task['message'] + ": " + task["detail"], False)
            return
